...
CREATE TABLE public."Recipe_Categories_Mapping" (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    recipe_id uuid NOT NULL,
    category_id uuid NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL
);
COMMENT ON TABLE public."Recipe_Categories_Mapping" IS 'Junction table handling many-to-many relationship mappings between Recipes and Categories/Tags.';
...
ALTER TABLE ONLY public."Recipe_Categories_Mapping"
    ADD CONSTRAINT "Recipe_Categories_Mapping_pkey" PRIMARY KEY (id);
...
ALTER TABLE ONLY public."Recipe_Categories_Mapping"
    ADD CONSTRAINT unique_recipe_category_link UNIQUE (recipe_id, category_id);
ALTER TABLE ONLY public."Bookmarks"
    ADD CONSTRAINT unique_user_recipe_bookmark UNIQUE (user_id, recipe_id);
ALTER TABLE ONLY public."Likes"
    ADD CONSTRAINT unique_user_recipe_like UNIQUE (user_id, recipe_id);
ALTER TABLE ONLY public."Ratings"
    ADD CONSTRAINT unique_user_recipe_rating UNIQUE (user_id, recipe_id);
CREATE INDEX idx_comments_parent_id ON public."Comments" USING btree (parent_id);
CREATE INDEX idx_rc_mapping_category ON public."Recipe_Categories_Mapping" USING btree (category_id);
CREATE INDEX idx_rc_mapping_recipe ON public."Recipe_Categories_Mapping" USING btree (recipe_id);
...
ALTER TABLE ONLY public."Recipe_Categories_Mapping"
    ADD CONSTRAINT "Recipe_Categories_Mapping_category_id_fkey" FOREIGN KEY (category_id) REFERENCES public."Categories"(id) ON DELETE CASCADE;
ALTER TABLE ONLY public."Recipe_Categories_Mapping"
    ADD CONSTRAINT "Recipe_Categories_Mapping_recipe_id_fkey" FOREIGN KEY (recipe_id) REFERENCES public."Recipes"(id) ON DELETE CASCADE;
ALTER TABLE ONLY public."Recipes"
